...
namespace Nanolod.Calibration.GA
{
    public class GeneticAlgorithm<IndividualType> : IMetaHeuristic<IndividualType>
    {
        ...
        public IndividualType GetBestIndividual()
        {
        ...
        {
            IEnumerable<IndividualType> individuals = Config.selectionFunction.Invoke(Population.Select((individual) => individual.key), Population.Select((individual) => individual.value), 2);
        ...
        line1:
            ...
            if (Config.writeToConsole && ((_iterationCount % Config.consoleWriteInterval) == 0) || (_iterationCount - 1 == 0))
            {
                if (Config.consoleWriteFunction == null)
                    Console.WriteLine(_iterationCount + "\t" + _iterationCount + " = " + _bestFitness);
                else
