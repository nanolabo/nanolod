...
namespace Nanolod.Calibration
{
    public class Distribution
    {
        ...
        private static double NextStdNormal()
        {
        ...
        {
            double sigma = Math.Pow(Gamma(1.0 * beta) * Math.Sin(Math.PI * beta / 2.0) / (Gamma((1.0 * beta) / 2.0) * beta * Math.Pow(2.0, (beta - 1.0) / 2.0)), 1.0 / beta);
            return alpha * NextStdNormal() * sigma / Math.Pow(Math.Abs(NextStdNormal()), 1.0 / beta);
        }
    }
}